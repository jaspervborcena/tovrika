...
export class PosComponent implements OnInit {
  ...
  // Only show stores that belong to the current user (by document id in user.storeIds)
  readonly availableStores = computed(() => {
    ...
    try {
      ...
      return all.filter(s => s.id && storeIds.includes(s.id));
    } catch (e) {
      return all;
    }
  });
  ...
  private async loadData(): Promise<void> {
    ...
    if (companyId) {
  // Load companies and stores first. Initialize selected store after stores are available,
  // then load products scoped to company and (optionally) the selected store.
  ...
  await this.productService.loadProductsByCompanyAndStore(companyId, this.selectedStoreId());
    } 
  }
