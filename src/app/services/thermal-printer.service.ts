...
export class ThermalPrinterService {
  ...
  constructor() {
    console.log('🖨️ Thermal Printer Service initialized');
  }
  ...
  async initialize(): Promise<boolean> {
    if (!Capacitor.isNativePlatform()) {
      console.log('⚠️ Not on native platform, thermal printing not available');
      return false;
    }
  ...
  async connectToPrinter(): Promise<boolean> {
    try {
      ...
      console.log('🔵 Connecting to:', device.name || device.deviceId);
      ...
      for (const service of services) {
        for (const char of service.characteristics) {
          if (char.properties.write || char.properties.writeWithoutResponse) {
            ...
            console.log('✅ Found write characteristic:', this.serviceUuid, this.writeCharacteristic);
            break;
          }
        }
  ...
  async printReceipt(receiptData: any): Promise<void> {
    ...
    try {
      ...
      console.log('🖨️ Printing receipt with data:', JSON.stringify(dataStructure, null, 2));
      console.log('🖨️ Full receipt data:', JSON.stringify(receiptData, null, 2));
  ...
  private async sendToPrinter(data: Uint8Array): Promise<void> {
    ...
    console.log(`🔧 Characteristic: ${this.writeCharacteristic}`);
  ...
  private generateESCPOS(receiptData: any): Uint8Array {
    ...
    console.log(`✅ Generated ${result.length} bytes of ESC/POS data`);
